...
void ssd1306_drawChar(unsigned char x, unsigned char y, char letter) {
    ...
    char hex, temp;
    for (i=0; i < 5; i++) {
        hex = ASCII[letter - 0x20][i];
        for (j=0; j<8; j++) {
            temp = (hex >> j) & 0b1;
            ssd1306_drawPixel(x+i, y+j, temp);
        }
    }
}

void ssd1306_drawString(unsigned char x, unsigned char y, char * string) {
    int i = 0;
    while (string[i] != 0) {
        ssd1306_drawChar(x + 5*i, y, string[i]);
        i++;
    }
}