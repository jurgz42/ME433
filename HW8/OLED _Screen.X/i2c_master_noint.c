...
void i2c_write(unsigned char reg, unsigned char val) {
    i2c_master_start();
    i2c_master_send(write_addy);    //send to address
    i2c_master_send(reg);           //send to register
    i2c_master_send(val);           //send to value
    i2c_master_stop();
}

unsigned char i2c_read(unsigned char reg) {
    i2c_master_start();
    i2c_master_send(write_addy);
    i2c_master_send(reg);
    i2c_master_restart();
    i2c_master_send(read_addy);
